...
'Declaring variables
Dim objRange, objExcel, objWorkbook, objWorksheet, objCell, strScriptDir, objFSO, strFile, oShell
...
MsgBox "The script will now start working in the '" + CStr(strScriptDir) + "' folder.  You will see another notification once the process has completed.", vbOKOnly, "Excel Tool"

Call CreateFolder("Completed")
Call CreateFolder("Final Report")
...
For Each strFile In objFSO.GetFolder(strScriptDir).Files
	...
	'Validate the filetype
		If strFilename = "Excel Tool" Then 
			'Do nothing
		ElseIf (strFileExtension = "xlsx") OR (strFileExtension = "xls") Then
			strExcelFilename = strScriptDir + "\" + "Final Report" + "\" + strFileName + " - Monthly Report.xlsx"
...
MsgBox "The process has completed!", vbOKOnly, "Excel Tool"
...
Function ExcelCopy(strFilename, strExcelFilename)
